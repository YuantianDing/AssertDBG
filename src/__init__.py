...
class TestResult:
    ...
    def show_result(self):
        result = TestResult()
        result.set_task_id(self.task_id, self.code_model, self.question_limit, show=True)
        for q, a in self.questions_and_answers:
            result.add_questions_and_answers(q, a)
        result.set_code(self.function_name, self.code, show=True)
...
class ResultDB:
    ...
    def run(self, testcase: TestCase, verbose: int = 0) -> TestResult:
        ...
        code = codegen.code_gen(testcase.prompt, verbose=verbose)
        result.set_code(testcase.entry_point, code, show = verbose > 2)

        result.set_testing(*testcase.run_test(code, testcase.entry_point), show=verbose > 1)
